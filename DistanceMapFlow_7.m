...
frames = 98; %Number of frames in image sequence
...
for k = 1:frames
    ...
    S3='.tif';
    if k < 10
        %SS1 = ['CNC Hydrophilic.lif_narrow not bidirectional_t00' num2str(k) '_z05_ch01'];
   % elseif k < 100
        SS1 = ['CNC Hydrophilic.lif_middle narrow 512 by 256 1 step size_Crop001_t0' num2str(k) '_ch00'];
    else
        SS1 = ['CNC Hydrophilic.lif_middle narrow 512 by 256 1 step size_Crop001_t' num2str(k) '_ch00'];
    end
...
for i = 1:2
    FiltIm = imfilter(FiltIm,ones(5,25)/5*25);
end
...
for l = frames:-1:2 %Going through frames
    ...
    for j = 1:length(tdatabackup)
        ...
        if l == frames && j == 1
            ...
            tdataprevbackup(:,4) = zeros(size(tdataprevbackup(:,1)));
            tdataprev(:,4) = zeros(size(tdataprevbackup(:,1)));
        end
        ...
        indx2 = round([tdata(j,2) - 165,tdata(j,2) + 165]);
        indy2 = round([tdata(j,3) - 165,tdata(j,3) + 165]);
        indx2(indx2<1) = 1;
        indx2(indx2>pixels(1)) = pixels(1);
        indy2(indy2<1) = 1;
        indy2(indy2>pixels(2)) = pixels(2);
        
        A = A(indx2(1):indx2(2),indy2(1):indy2(2));
        ...
        corr_offset = [ (ypeak-size(T,1)/2-size(A,1)/2) (xpeak-size(T,2)/2-size(A,2)/2) ];
    ...
    for m = 1:min(length(tdataprevbackup),length(tdatabackup))
        ...
        indx2 = round([tdata(BestCentDiffm,2) - 165,tdata(BestCentDiffm,2) + 165]);
        indy2 = round([tdata(BestCentDiffm,3) - 165,tdata(BestCentDiffm,3) + 165]);
        indx2(indx2<1) = 1;
        indx2(indx2>pixels(1)) = pixels(1);
        indy2(indy2<1) = 1;
        indy2(indy2>pixels(2)) = pixels(2);
...
%         end
    ...
    for n = 1:length(CheckThing)
        ...
        indx2 = round([tdataprevbackup(n2,2) - 165,tdataprevbackup(n2,2) + 165]);
        indy2 = round([tdataprevbackup(n2,3) - 165,tdataprevbackup(n2,3) + 165]);
        indx2(indx2<1) = 1;
        indx2(indx2>pixels(1)) = pixels(1);
        indy2(indy2<1) = 1;
        indy2(indy2>pixels(2)) = pixels(2);
        
        A2 = A2(indx2(1):indx2(2),indy2(1):indy2(2));
        T2= FiltIm(indx(1):indx(2),indy(1):indy(2),l-1);
        
        cc = normxcorr2(T2,A2);
        [max_cc, imax] = max(abs(cc(:)));
        [ypeak, xpeak] = ind2sub(size(cc),imax(1));
        corr_offset = [ (ypeak-size(T2,1)/2-size(A2,1)/2) (xpeak-size(T2,2)/2-size(A2,2)/2) ];
