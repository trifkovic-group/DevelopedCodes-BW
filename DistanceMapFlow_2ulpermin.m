...
frames = 10; %Number of frames in image sequence
...
for k = 1:frames
    ...
    S3='.tif';
    if k < 10
        SS1 = ['FlowForDistanceMap.lif_2ulpermin_Crop_GS_001_t00' num2str(k) '_z3_ch01'];
    elseif k < 100
        SS1 = ['FlowForDistanceMap.lif_2ulpermin_Crop_GS_001_t0' num2str(k) '_z3_ch01'];
    else
        SS1 = ['FlowForDistanceMap.lif_2ulpermin_Crop_GS_001_t' num2str(k) '_z3_ch01'];
    end
...
for i = 1:length(paths)
    p = cell2mat(paths(i));
    if length(p(:,1)) > 10 && length(p(:,1)) < 25
        for m = length(p(:,1)):-1:2
            if p(m,3)<p(m-1,3) && p(m-1,3) - p(m,3) < 80 && abs(p(m,2) - p(m-1,2)) < 30
            quiver(p(m,3),p(m,2),-(p(m,3)-p(m-1,3)),-(p(m,2)-p(m-1,2)),'Color',[abs(1-colorVal) 0 colorVal])
            Angs(AngCount) = atan((p(m,2)-p(m-1,2))/(p(m,3)-p(m-1,3)));
            AngCount = AngCount+1;
            end
        end
        colorVal = colorVal+.02;
        for j = 1:length(p(:,1))
