...
frames = 72; %Number of frames in image sequence

for k = 1:frames
    
    if k < 10
        CH0 = ['Diluted nanoemulsion.lif_Series015_t0' num2str(k) '_ch00.tif'];
    else
        CH0 = ['Diluted nanoemulsion.lif_Series015_t' num2str(k) '_ch00.tif'];
    end
    
    if k < 10
        CH1 = ['Diluted nanoemulsion.lif_Series015_t0' num2str(k) '_ch01.tif'];
    else
        CH1 = ['Diluted nanoemulsion.lif_Series015_t' num2str(k) '_ch01.tif'];
    end
...
Ch1BF = imfilter(Ch1,ones(5)/25);
Ch0BW = imbinarize(Ch0BF);
Parties = zeros(128,128,frames);

for i = 1:frames
    BW = Ch0BW(:,:,i);
    PKiller = zeros(128,128);
    DM = bwdist(1-BW);
    Psize = max(max(DM));
    Pinfo = [];
    Pcount = 1;
    Partie = zeros(128,128);
    while Psize > 1.1
        DM = bwdist(1-BW);
        Psize = max(max(DM));
...
for i = 1:length(Tracked)-1
    ...
    if length(TrN) * length(TrC) > 0
        for j = 1:length(TrC(:,1))
            ...
            [CX,CY] = ind2sub([128,128],TrC(j,2));
            [NX,NY] = ind2sub([128,128],TrN(M,2));
            
            if length(M) > 0
                D(length(D)+1) = sqrt((CX-NX)^2 + (CY-NY)^2);
                quiver(CX/10,CY/10,(NX-CX)/10,(NY-CY)/10,'color',[1-TrC(j,3)/150,TrC(j,3)/150,1-TrC(j,3)/150],'MaxHeadSize',1)
            end
        end
    end
end
